...
def calculate_demographic_data(print_data=True):
    ...
    average_age_men = ((df.loc[filter_men, 'age']).mean().round(1))

    # What is the percentage of people who have a Bachelor's degree?
    nun_bachelors = len(df[df['education'] == 'Bachelors'])
    total_nun = len(df)
    percentage_bachelors = round(nun_bachelors / total_nun * 100, 1)

    # What percentage of people with advanced education (`Bachelors`, `Masters`, or `Doctorate`) make more than 50K?
    # What percentage of people without advanced education make more than 50K?

    # with and without `Bachelors`, `Masters`, or `Doctorate`
    higher_education = df[df['education'].isin(['Bachelos','Master','Doctorate'])]
    lower_education = df[~df['education'].isin(['Bachelos','Master','Doctorate'])]
    ...
    highest_earning_country = (country_rich_count / country_count * 100).idmax()
    ...
    occupation_counts = people_of_india[people_of_india['occupation'].value_counts()]
